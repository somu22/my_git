...
def Database():
    ...
    conn = mysql.connector.connect(user='root', password='hpc@123', host='192.168.100.168',database='mydb1')
    cursor = conn.cursor()
    cursor.execute("CREATE TABLE IF NOT EXISTS `admin`(admin_id INTEGER PRIMARY KEY NOT NULL AUTO_INCREMENT,username TEXT,password TEXT)")
    cursor.execute("CREATE TABLE IF NOT EXISTS `product`(product_id INTEGER PRIMARY KEY NOT NULL AUTO_INCREMENT , product_name TEXT, product_qty TEXT, product_price TEXT)")
...
#	btn_addquantity = Button(LeftViewForm, text="Add Quantity", command=addquantity)
#   btn_addquantity.pack(side=TOP, padx=10, pady=10, fill=X)
...
#   else:
#        result = tkMessageBox.askquestion('Simple Inventory System', 'Are you sure you want to add this quantity?', icon="warning")
#        if result == 'yes':
#            curItem = tree.focus()
...
#            tree.delete(curItem)
#            Database()
#            cursor.execute("UPDATE `product` WHERE `product_id` = %d" % selecteditem[0])
#            conn.commit()
#            cursor.close()
#            conn.close()
